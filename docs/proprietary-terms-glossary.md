...
- **37-Lens Sweep** — the one-per-session pass where every lens answers the verbatim SMP. It fires **once, before any channel brief exists**, against the validated proposition/Detonation (`startBigIdeaRun`), then Tissue Check triage → Gate One approval → `lockWinningIdea`. Channel Detonation Briefs and all downstream channel/martech prompts are generated from that locked idea only. A legacy per-channel sweep (one run per channel brief) was retired in Sep 2026 and is historical, not current — see `docs/creative-stimulus-architecture.md`.
