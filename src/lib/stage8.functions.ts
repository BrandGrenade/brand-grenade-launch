...
export const runStage8 = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ output: string }> => {
    ...
    try {
      resp = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 8192,
          temperature: 0.7,
          system: STAGE_8_SYSTEM_PROMPT,
          messages: [{ role: "user", content: userMessage }],
        }),
      });
    } catch (e) {
      const msg = e instanceof Error ? e.message : "network error";
      await supabaseAdmin.from("sessions").update({ stage_8_error: `Network: ${msg}` }).eq("id", data.sessionId);
    ...
    if (!resp.ok) {
      ...
      await supabaseAdmin.from("sessions").update({ stage_8_error: msg }).eq("id", data.sessionId);
      throw new Error(msg);
    }
    ...
    const output = (json.content ?? [])
      ...
      .trim();
    if (!output) {
      const msg = "Claude returned an empty Stage 8 response";
      await supabaseAdmin.from("sessions").update({ stage_8_error: msg }).eq("id", data.sessionId);
      throw new Error(msg);
    }
