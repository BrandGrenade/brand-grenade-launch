...
export const runStage11 = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ output: string }> => {
    ...
    try {
      ...
      await supabaseAdmin.from("sessions").update({ stage_11_error: `Network: ${msg}` }).eq("id", data.sessionId);
    ...
    if (!resp.ok) {
      ...
      await supabaseAdmin.from("sessions").update({ stage_11_error: msg }).eq("id", data.sessionId);
      throw new Error(msg);
    }
    ...
    const output = (json.content ?? [])
      ...
      .trim();
    if (!output) {
      const msg = "Claude returned an empty Stage 11 response";
      await supabaseAdmin.from("sessions").update({ stage_11_error: msg }).eq("id", data.sessionId);
      throw new Error(msg);
    }
