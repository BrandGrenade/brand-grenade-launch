...
export function deriveMintoContent(session: MintoSession, opts: DeriveOptions = {}): DerivedMinto {
  ...
  // Every count printed anywhere in this document comes from run facts, and
  // each one is labelled for exactly what it counts. The front matter and the
  // system-proof footer therefore cannot disagree: "considered" and
  // "competitively scored" are separate figures, never the same stat renamed.
  const consideredCount = runFacts.propositionsConsidered;
  const scoredCount = runFacts.propositionsScored;
  if (consideredCount) {
    headlineStats.push({
      value: consideredCount,
      label: "Propositions considered",
      note: `${scoredCount} competitively scored · ${passed.length} cleared the hard floors.`,
    });
  }
  headlineStats.push({
    value: `${stagesRun}/${runFacts.stagesTotal}`,
    label: "Pipeline stages run",
    note: `Of ${runFacts.stagesTotal} numbered stages; any not run belong to phases this session has not reached. Each completed stage is evidenced in the appendix.`,
  });
  ...
  const background =
    `<p>This document was generated by the Brand Grenade Strategy Intelligence System for <strong>${escapeHtml(
      brand,
    )}</strong>${category ? ` in the ${escapeHtml(category)} category` : ""}. It is assembled entirely from one strategy session: ${stagesRun} of ${runFacts.stagesTotal} validation stages were run and are reproduced in the appendix, ${consideredCount || "no"} proposition${
      consideredCount === 1 ? "" : "s"
    } were generated and ${scoredCount} competitively scored, and the recommendation above is ${
