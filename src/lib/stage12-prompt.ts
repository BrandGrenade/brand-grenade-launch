...
Weighted Composite: [n]/90

Copy every number VERBATIM from the FROZEN STAGE 10 SCORES block in the user message — same six labels, same weights, same weighted composite out of 90. Do NOT use the legacy seven-dimension labels (Differentiation, Cultural Relevance, Fame Potential, Writer Quality, Commercial Plausibility, Creative Expandability) and do NOT emit a /70 composite. If the frozen block marks a proposition "SCORES UNAVAILABLE", render "Scores: not available" verbatim in place of the score rows.
