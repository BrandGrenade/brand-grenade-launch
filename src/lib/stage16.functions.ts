...
MASTER DETONATION LINE — THE SHORT HEADLINE FROM STAGE 18 (this is the short punchy headline the human selected — for example "BRING YOUR HARDEST BRIEF". This is the ONLY value that may appear in the hero H1 line under the THE DETONATION heading. It is never the long explanatory statement. Output it verbatim — exact words, exact casing, no quotes added, no trailing punctuation added):
${(session as Record<string, unknown>).stage_18_detonation_line ?? "[No master detonation line selected yet — derive the short organising headline from the Brand World Definition and Creative Territory Mapping above. Do not substitute a springboard direction.]"}

MASTER DETONATION STATEMENT — THE LONGER EXPLANATORY SENTENCE FROM STAGE 18 (this is the supporting paragraph that explains what the detonation line means. It belongs in normal body text AFTER the hero line, never inside the hero H1):
${(session as Record<string, unknown>).stage_18_selected_detonation ?? "[No master detonation statement selected yet — write the explanatory paragraph from the Brand World Definition and Creative Territory Mapping above.]"}
