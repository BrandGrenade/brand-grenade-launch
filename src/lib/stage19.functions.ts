...
export const runStage19 = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { data: session, error } = await supabaseAdmin
      .from("sessions")
      .select("brand_name, category, stage_18_selected_detonation, stage_18_output, stage_14b_output, truth_product, truth_consumer, truth_cultural, stage_19_output")
    ...
    const { error: saveErr } = await supabaseAdmin
      .from("sessions")
      .update({ stage_19_output: output, stage_19_error: null, phase_2_current_stage: 19 })
      .eq("id", data.sessionId);
