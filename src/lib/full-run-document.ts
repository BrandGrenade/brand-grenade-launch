...
export function buildFullRunDocument(session: FullRunSession): string {
  ...
  // Universal standard — every document states its own background, and every
  // document closes with a decisive next step.
  const background = `<div class="stage-block"><div class="part-label">BACKGROUND</div><h2>Background and context</h2>
<p>This is the <strong>Complete Pipeline Run</strong> for ${escapeHtml(brand)} — the full canonical record of one Brand Grenade strategy session, generated by the Brand Grenade Strategy Intelligence System from that session's stored stage outputs and nothing else.</p>
<p>${resolved.length} stage${resolved.length === 1 ? "" : "s"} produced output and ${resolved.length === 1 ? "is" : "are"} reproduced here in full, in the order they were run. Stages with no recorded output are omitted rather than summarised.</p>
<p>It exists as the audit record behind the strategy: it serves anyone who needs to see how the recommendation was reached, stage by stage, rather than only the conclusion.</p></div>`;
