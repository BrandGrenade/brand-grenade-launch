...
export const loadStimulusRun = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: directions, error } = await supabaseAdmin
      .from("stimulus_directions")
      .select(
        "id, lens_id, lens_name, sort_order, direction, campaign_line, expression_under_master, master_line_at_generation, rationale, line_check, status, instinct_brief, revise_notes, revise_count, error, ratings, rating_status, rating_error, rated_at, gate_one_approved, gate_one_approved_at, gate_one_notes",
      )
      .eq("run_id", run.id)
      .order("sort_order", { ascending: true });
    if (error) throw new Error(error.message);
