...
export const SPECS_C: Spec[] = [
  ...
  { id: "cs-bigidea", name: "Big idea sweep batch", factBound: true,
    ...
    check: (out) => { const { parseBigIdeaResponse } = require("../src/lib/stimulus/big-idea-prompt"); const ids = (globalThis as any).__lensIds as string[]; let got: string[] = []; try { const p = parseBigIdeaResponse(out, ids); got = ids.filter((i) => p?.[i]); } catch {} return { pass: got.length === ids.length, detail: `parsedLenses=${got.length}/${ids.length}` }; } },
  { id: "cs-ledger", name: "Convergence ledger", factBound: true,
  ...
  { id: "cs-channel", name: "Channel adaptation", factBound: true, sessionId: DM, cols: "brand_name, category, selected_smp, stage_21_outputs, locked_big_idea, locked_campaign_line, locked_big_idea_lens",
  ...
  { id: "cs-offline", name: "Offline creative brief", factBound: true, sessionId: DM, cols: "brand_name, category, selected_smp, stage_21_outputs, locked_big_idea, locked_campaign_line, locked_big_idea_lens, stage_20_output, brand_constraints, brand_tone_of_voice",
